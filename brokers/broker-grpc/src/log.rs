pub mod logs {
    ...
    pub fn init_logs() {
        ...
        let strong_level_ap = RollingFileAppender::builder()
            .encoder(Box::new(PatternEncoder::new(roller_pattern)))
            .build(
                "logs/strong_level/strong_level.log",
                Box::new(CompoundPolicy::new(trigger.clone(), roller.clone())),
            )
            .unwrap();
        ...
        log4rs::init_config(config).expect("Error to configure logs");
    }
}
